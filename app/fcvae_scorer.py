...
class FCVAEScorer:
    ...
    def __init__(self, config: Optional[FCVAEScorerConfig] = None):
        ...
        self.point_threshold: Optional[float] = None  # Per-point threshold (all positions)
        self.last_point_threshold: Optional[float] = None  # Position [-1] threshold (paper-faithful)
    ...
    def set_last_point_threshold(
        ...
        Per the paper, the FCVAE frequency conditioning masks position [-1],
        making it the only genuine prediction. The score distribution at [-1]
        differs from positions [0..22] (reconstruction scores), so the threshold
        should be calibrated specifically on last-point scores.

        Args:
            point_scores: Per-point scores, shape (N, W) where W is window_size.
                         Only position [-1] is used.
            method: "percentile" (default from config)
            percentile: Percentile value (default from config)

        Returns:
            Last-point threshold τ
        """
        method = method or self.config.threshold_method
        percentile = percentile if percentile is not None else self.config.threshold_percentile

        # Extract position [-1] scores only
        if point_scores.ndim == 2:
            last_point_scores = point_scores[:, -1]
        else:
            last_point_scores = point_scores  # Already 1D
    ...
    def find_optimal_last_point_threshold(
        ...
        Like find_optimal_threshold but operates on last-point scores,
        which have a different distribution than all-position scores
        due to the frequency conditioning masking.

        Args:
            normal_scores: Position [-1] scores from normal windows, shape (N,)
            anomaly_scores: Position [-1] scores from anomaly windows, shape (N,)
            method: Calibration method ("f1_max", "midpoint", "youden")
            beta: Beta for F-beta score

        Returns:
            Tuple of (optimal_threshold, calibration_metrics)
        """
        # Delegate to the general method — it works on 1D arrays
