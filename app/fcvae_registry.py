...
class FCVAERegistry:
    ...
    def fit_scorer_with_calibration(
        ...
        # Last-point threshold (paper-faithful): calibrate on position [-1] only
        # Position [-1] is masked in frequency conditioning, giving it a different
        # score distribution than positions [0..22] (reconstruction scores)
        normal_lp_scores = normal_point_scores[:, -1]
        anomaly_lp_scores = anomaly_point_scores[:, -1]
        anomaly_lp_labels = anomaly_point_labels[:, -1]

        lp_true_anomaly = anomaly_lp_scores[anomaly_lp_labels == 1]
        lp_normal_from_anomaly = anomaly_lp_scores[anomaly_lp_labels == 0]
        all_lp_normal = np.concatenate([normal_lp_scores, lp_normal_from_anomaly])

        if len(lp_true_anomaly) > 0:
            lp_threshold, lp_metrics = FCVAEScorer.find_optimal_threshold(
                all_lp_normal, lp_true_anomaly, method="f1_max"
            )
            scorer.last_point_threshold = lp_threshold
            logger.info(f"    Last-point threshold (F1-max): {lp_threshold:.4f} "
                       f"(vs all-position: {optimal_point_threshold:.4f})")
        else:
            # Fallback: use percentile on position [-1]
            scorer.last_point_threshold = float(np.percentile(normal_lp_scores, 5.0))
            logger.info(f"    Last-point threshold (percentile fallback): {scorer.last_point_threshold:.4f}")
        ...
        try:
            from evaluate_fcvae import plot_anomaly_reconstruction
            from pathlib import Path
        ...
        return {
            ...
            "last_point_threshold": float(scorer.last_point_threshold) if scorer.last_point_threshold else None,
