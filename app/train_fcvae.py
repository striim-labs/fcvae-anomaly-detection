...
def optimize_threshold_f1(
    ...
    # --- All-position threshold (backward compatibility) ---
    ...
    # Its score distribution differs from positions [0..22] (reconstruction scores).
    ...
    if len(lp_anomaly) > 0:
        ...
        logger.info(
            f"  Last-point F1-optimized threshold: {lp_threshold:.4f} "
            f"(vs all-position: {optimal_threshold:.4f}, delta={lp_threshold - optimal_threshold:+.4f})"
        )
    else:
        # Fallback: use percentile on position [-1] scores
        scorer.set_last_point_threshold(all_point_scores, method="percentile", percentile=5.0)
        metrics["last_point_threshold"] = scorer.last_point_threshold
        metrics["last_point_method"] = "percentile_fallback"
        logger.info(f"  No anomalies at position [-1], using percentile: {scorer.last_point_threshold:.4f}")

    # --- Window threshold (for legacy decision modes) ---
