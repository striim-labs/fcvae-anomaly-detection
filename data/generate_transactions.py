...
# For dip anomalies, combo_hours overrides the default hours per-combo
# so the dip lands at each combo's peak hour for maximum detectability.
TEST_ANOMALIES = [
...
def inject_spike_anomaly(
    ...
    When combo is provided, scales the effective multiplier per-hour so the
    absolute transaction count increase is proportional to the combo's peak
    rate, regardless of where in the sinusoidal cycle the spike lands.

    Args:
        day_params: Day-specific variability parameters (modified in place)
        hours: Hours to inject spike
        multiplier: Rate multiplier for spike hours (at peak; scaled elsewhere)
        combo: Optional combo key for phase-aware scaling
    ...
    for h in hours:
        ...
        if combo is not None:
            ...
            rate_h = params["base_rate"] + params["amplitude"] * np.sin(
                2 * np.pi * h / params["period_h"] + params["phase"]
            )
            peak_rate = params["base_rate"] + params["amplitude"]
            # Floor to avoid extreme multipliers at deep troughs
            rate_h = max(rate_h, params["base_rate"] * 0.2)
            # Scale so absolute increase ≈ what it would be at the peak
            effective_mult = 1 + (multiplier - 1) * peak_rate / rate_h
        day_params["hourly_noise"][h] *= effective_mult
    return hours
...
def inject_gradual_ramp(
    ...
    When combo is provided, scales the effective multiplier per-hour so the
    absolute increase is proportional to the combo's peak rate.
    ...
    for i, h in enumerate(hours):
        ...
        if combo is not None:
            ...
            peak_rate = params["base_rate"] + params["amplitude"]
            rate_h = max(rate_h, params["base_rate"] * 0.2)
            mult = 1 + (mult - 1) * peak_rate / rate_h
...
def inject_anomaly(
    ...
    Args:
        ...
        combo: Optional combo key; when provided, dip hours are overridden
               with the combo's peak hour for maximum detectability.
    ...
    if anomaly_type == "spike":
        ...
        hours = anomaly_config["hours"]
        combo_hours = anomaly_config.get("combo_hours", {})
        if combo is not None and combo in combo_hours:
            hours = combo_hours[combo]
    ...
    for combo in COMBO_KEYS:
        ...
        for day in range(total_days):
            ...
            if day in anomaly_by_day:
                ...
                anomaly_hours = inject_anomaly(day_params, anomaly_config, combo=combo)
